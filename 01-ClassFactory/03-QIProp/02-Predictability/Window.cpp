...
*/
		...
		// Checking for Predictability property of query interface
		Sleep(5000);
		hr = pISum->QueryInterface(IID_ISubtract, (void**)&pISubtractAgain);
		if (FAILED(hr))
		{
		...
		{
			MessageBox(NULL, TEXT("Predictability true"), TEXT("Error"), MB_OK);
		}

		else 
		{
			MessageBox(NULL, TEXT("Predictability not true"), TEXT("Error"), MB_OK);
		}
...
void SafeInterfaceRelease(void)
{
	...
	if (pISubtractAgain)
	{
		pISubtractAgain->Release();
		pISubtractAgain = NULL;
	}
