...
ISubtract* pISubtractAgain = NULL;
...
*/
		...
		// Checking for Reflexibility property of query interface
		hr = pISum->QueryInterface(IID_ISum, (void**)&pISumAgain);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to obtained ISum from ISum itself"), TEXT("Error"), MB_OK);
			
			
			DestroyWindow(hwnd);
		}

		hr = pISubtract->QueryInterface(IID_ISubtract, (void**)&pISubtractAgain);
		if (FAILED(hr))
		{
			MessageBox(hwnd, TEXT("Failed to obtained ISubtract from ISubtract itself"), TEXT("Error"), MB_OK);
		...
		if (pISum == pISumAgain)
		{
			MessageBox(NULL, TEXT("Reflexibility true"), TEXT("Error"), MB_OK);
		}

		else 
		{
			MessageBox(NULL, TEXT("Reflexibility not true"), TEXT("Error"), MB_OK);
		}

		if (pISubtract == pISubtractAgain)
		{
			MessageBox(NULL, TEXT("Reflexibility true"), TEXT("Error"), MB_OK);
		}
